# Social Card Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the `SocialCarousel` swipeable card stack on the main public page with a `SocialCard` grid component that matches the visual style of the provided reference component, using dynamic DB-driven social links.

**Architecture:** Create `src/components/public/SocialCard.tsx` as a client component receiving `SocialLink[]` props. It renders a glass-style card with up to 4 social link boxes in a 2×2 grid with staggered hover animations, all in Tailwind. Then update `src/app/page.tsx` to swap `SocialCarousel` for `SocialCard`.

**Tech Stack:** Next.js 15, React, Tailwind CSS, TypeScript, next/image, `@/lib/icons` (getIcon, ICON_IMAGE_MAP)

---

### Task 1: Create `SocialCard` component

**Files:**
- Create: `src/components/public/SocialCard.tsx`
...
Run: `cat src/components/public/SocialCard.tsx | head -5`
Expected: `"use client";` on line 1

**Step 3: Commit**

```bash
git add src/components/public/SocialCard.tsx
git commit -m "feat: add SocialCard 2x2 grid component with dynamic DB links"
```

---

### Task 2: Swap `SocialCarousel` for `SocialCard` in the main page
...
In `src/app/page.tsx`:
- Line 6: change `import SocialCarousel from "@/components/public/SocialCarousel";` → `import SocialCard from "@/components/public/SocialCard";`
- In the JSX (around line 52–55): replace the `<SocialCarousel links={links} />` block with `<SocialCard links={links} />`
...
Expected: no TypeScript or import errors. (Or run `npx tsc --noEmit` for a faster type check.)

**Step 3: Commit**

```bash
git add src/app/page.tsx
git commit -m "feat: replace SocialCarousel with SocialCard on main public page"
```

---

### Task 3: Copy component to `/components/ui` per the task requirement

**Files:**
- Create: `src/components/ui/social-card.tsx` (re-export from the canonical location)

**Step 1: Create the re-export shim**

```tsx
// Re-export from canonical location for shadcn convention compatibility
export { default } from "@/components/public/SocialCard";
```
...
git commit -m "chore: add shadcn-path re-export for SocialCard"
```
